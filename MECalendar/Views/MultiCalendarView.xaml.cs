...
{
    public partial class MultiCalendarView : ContentView
    {
        ...
        {
            var control = (MultiCalendarView)bindable;
        ...
        public CalendarType SecondaryCalendarType
        {
            get { return (CalendarType)GetValue(SecondaryCalendarTypeProperty); }
            set { SetValue(SecondaryCalendarTypeProperty, value); }
        }

        public static BindableProperty SecondaryCalendarTypeProperty = BindableProperty.Create(
            nameof(SecondaryCalendarType),
            typeof(CalendarType),
            typeof(MultiCalendarView),
            CalendarType.Gregorian,
            propertyChanged: SecondaryCalendarTypeChanged
        );

        static void SecondaryCalendarTypeChanged(BindableObject bindable, object oldValue, object newValue)
        {
            var control = (MultiCalendarView)bindable;
            control.SecondaryCalendarType = (CalendarType)newValue;
            control.InitializeCalendarView();
        }
        ...
        void PopulateGregorianCalendar(int monthsToAdd)
        {
            ...
            lbl_sec_month.Text = GetArabicNumbers(firstDayOfMonth.ToString("MMM yyyy", new CultureInfo("ar-SA").DateTimeFormat));
            ...
            {
                var calendarCell = grd_calendar.Children.ElementAt(index) as MultiCalendarCell;
                ...
                calendarCell.GestureRecognizers.Add(new TapGestureRecognizer((View arg1, object arg2) =>
                {
                    ...
                    {
                        var prevSelectedCell = grd_calendar.Children.ElementAt(indexOfLastSelectedDate) as MultiCalendarCell;
                        prevSelectedCell.Color = colorOfLastSelectedDate;
                    }
                    var arg = new DateSelectionArgs();
                    arg.SelectedDate = (arg1 as MultiCalendarCell).Date;
                    arg.Events = (arg1 as MultiCalendarCell).Events;
                    colorOfLastSelectedDate = (arg1 as MultiCalendarCell).Color;
                    indexOfLastSelectedDate = grd_calendar.Children.IndexOf(arg1 as MultiCalendarCell);
                    (arg1 as MultiCalendarCell).Color = SelectedDayColor;
                    DateSelected?.Execute(arg);
                }));

                calendarCell.Day = i.ToString();
                var equivalendSecondaryDate = new DateTime(CurrentDateInfo.Year, CurrentDateInfo.Month, i).ToString("%d", new CultureInfo("ar-SA").DateTimeFormat);
                if (equivalendSecondaryDate.Equals("1") && i != 1)
                    lbl_sec_month.Text += " - " + GetArabicNumbers(firstDayOfMonth.ToString("MMM yyyy", new CultureInfo("ar-SA").DateTimeFormat));

                calendarCell.SecondaryDay = GetArabicNumbers(equivalendSecondaryDate);
